...
export default function Brochure() {
  ...
  useEffect(() => {
    // On web, require()'ing a non-image asset resolves directly to its
    // final URL string (not a module id), so Asset.fromModule mishandles
    // it — use the string as-is instead of routing through Asset.
    if (Platform.OS === 'web') {
      // pdf.js resolves the url against document.baseURI, which is
      // "about:srcdoc" inside our iframe — a root-relative path can't
      // resolve against that, so make it absolute against our own origin.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setPdfUri(new URL(brochurePdf as unknown as string, window.location.href).toString());
      return;
    }
        ...
        {pdfUri ? (
          Platform.OS === 'web' ? (
            React.createElement('iframe', {
              srcDoc: getPdfHtml(pdfUri, pdfJsUri, pdfWorkerUri),
              sandbox: 'allow-scripts allow-same-origin',
              style: { width: '100%', height: '100%', border: 'none', borderRadius: 16, backgroundColor: '#ffffff' },
